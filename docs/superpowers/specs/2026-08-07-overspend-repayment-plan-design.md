...
### 4-3. 급여일 정산 (2026-08-07 개정)

> **개정 사유.** 최초 설계는 "급여일에 부채를 리셋하지 않음"이었다. 구현 후 수치로 검증하니
> **사용자가 평소 씀씀이를 유지하면 부채가 영원히 줄지 않았다.** 기본 예산 100,000원 · 20% 상환이면
> 실제 가용액은 80,000원인데 습관대로 100,000원을 쓰면 매일 20,000원이 초과되어 다시 부채로 합산되고,
> 상환액과 정확히 상쇄된다. 19일을 돌려도 부채가 280,000원에서 1원도 줄지 않았다.
> 화면도 계속 음수로 보여, 기능이 해결하려던 문제가 그대로 재현됐다.
...
- `BudgetConfig.absorbedDebtAmount` / `absorbedDebtPeriodStart`에 저장하고 **해당 기간에만** 차감
- 흡수한 만큼 부채를 상환 처리(원장 기록)하고 종료
- **순서 주의**: 전날 초과분을 부채에 합산한 **뒤에** 흡수해야 급여일 전날 초과까지 정산된다
- 부채가 흡수 가능액보다 크면 감당할 만큼만 흡수, 나머지는 다음 급여일로. 하루 예산은 음수가 되지 않는다
- 기간당 1회 (멱등)

페널티는 "하루 예산이 줄어드는" 형태로 유지되면서, 부채는 매 급여 기간 반드시 정산된다.

### 4-4. 상환 속도 경고

씀씀이를 줄이지 않으면 어떤 규칙으로도 적자는 줄지 않는다. 그 사실을 사용자에게 알린다.
...
최근 7일 평균 소비(오늘 제외)가 `기본 예산 − 하루 상환액`을 넘으면 홈 부채 카드에
"이 속도면 초과분이 줄지 않아요. 하루 N원씩 더 줄여보세요."를 표시한다.
...
- `absorbedDebtAmount: Int32` (기본 0) — 급여일에 이 기간 예산으로 흡수한 부채액
- `absorbedDebtPeriodStart: Date?` — 위 금액이 적용되는 급여 기간 시작일
...
| 급여일 도래 | 남은 부채를 새 기간 예산으로 흡수 후 종료 (4-3) |
| 부채가 한 달 배분 가능액보다 큼 | 감당할 만큼만 흡수, 나머지는 다음 급여일로. 하루 예산은 0 미만이 되지 않음 |
| 흡수 후에도 습관을 안 바꿈 | 매일 다시 초과 → 부채 재발생. 3-8 경고로 "하루 N원 더 줄이기" 안내 |
...
**소급 처리**
19. 앱을 며칠 안 열어도 날짜별로 상환 소급 적용
20. 소급 처리 반복 호출 시 중복 상환 없음

**급여일 정산 (4-3)**
21. 급여일에 부채가 흡수되고 하루 예산이 줄어듦 (음수 아님)
22. 흡수는 해당 급여 기간에만 적용, 다음 기간에는 사라짐
23. 기간당 1회만 (멱등)
24. 부채가 한 달치보다 크면 감당할 만큼만 흡수하고 나머지는 남김

> 테스트는 실효 급여일이 **주말이면 금요일로 당겨진다**는 점을 반영해,
> "최근 며칠 중 실효 급여일이 정확히 그날인" 오프셋을 찾아 사용한다.
> "오늘 = 급여일"로 단정하면 주말에 실패한다.

**상환 속도 경고 (4-4)**
25. `spendableWhileRepaying` — 기본 예산 − 상환액
26. `isOffTrack` — 평균 소비가 위 금액을 넘으면 true
27. `dailyCutNeeded` — 더 줄여야 하는 금액 (이미 줄고 있으면 0)
28. `recentAverageDailySpending` — 오늘 제외, 일수로 나눔

## 9. 범위 제외 (v-next)

- 남은 양수 잔액으로 부채 자동 조기 상환
- 부채 이력 화면 (완납한 부채 목록)
- 급여 기간을 넘는 대형 부채의 예산 재설정 유도 (지금은 흡수 후 남은 만큼 다음 기간으로)
